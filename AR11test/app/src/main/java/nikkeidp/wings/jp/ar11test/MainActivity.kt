package nikkeidp.wings.jp.ar11test
...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private  fun createModel(anchorNode: AnchorNode,selected: Int) {
        ...
        if(selected==1){
            ...
            val pin = TransformableNode(arFragment.transformationSystem)
            pin.setParent(anchorNode)
            pin.renderable = pinRenderable
            pin.select()

            addName(anchorNode,pin,"pin");
        }
    }
    ...
    private fun setupModel() {
      ModelRenderable.builder().
        ...
        ModelRenderable.builder().
            setSource(this@MainActivity,R.raw.pin)
            .build()
            .thenAccept{ modelRenderable -> pinRenderable = modelRenderable  }
            .exceptionally{ throwable ->
                Toast.makeText(this@MainActivity,"Unnable to load pin model",Toast.LENGTH_SHORT).show()
                null
            }
